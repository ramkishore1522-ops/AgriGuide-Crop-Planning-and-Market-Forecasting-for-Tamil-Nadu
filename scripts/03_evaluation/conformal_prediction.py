...
def split_conformal_prediction(
    ...
    for alpha in alpha_levels:
        confidence = 1 - alpha
        # Conformal quantile
        q_level = np.ceil((1 - alpha) * (len(nonconformity_scores) + 1)) / len(
        ...
        print(
            f"    {commodity_name} [{label}]: Coverage={coverage * 100:.1f}%, "
...
def visualize_intervals(all_results: list) -> None:
    ...
    for idx, result in enumerate(all_results):
        ...
        ax.plot(
            ...
            "r--",
            linewidth=1.5,
            label=f"Predicted (R²={result['point_R2']:.3f})",
        )
    ...
    plt.suptitle(
        "Conformal Prediction Intervals by Commodity", fontsize=14, fontweight="bold"
    )
    plt.tight_layout()
    plt.savefig(
        VISUALIZATIONS / "figure_prediction_intervals.png", dpi=300, bbox_inches="tight"
    )
    plt.close()
...
def visualize_coverage(summary_df: pd.DataFrame) -> None:
    ...
    if "coverage_90%" in summary_df.columns:
        ...
        bars_95 = summary_df.get(
            "coverage_95%", pd.Series([0] * len(commodities))
        ).values
    ...
    plt.suptitle(
        "Conformal Prediction: Coverage & Width Analysis",
        fontsize=14,
        fontweight="bold",
    )
    plt.tight_layout()
    plt.savefig(
        VISUALIZATIONS / "figure_coverage_analysis.png", dpi=300, bbox_inches="tight"
    )
    plt.close()
