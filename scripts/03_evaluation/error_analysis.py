            ...
            all_preds.append(
                {
                    ...
                    "pct_error": float(
                        abs(y[ti] - y_pred[i]) / max(abs(y[ti]), 1e-6) * 100
                    ),
                }
            )
...
def analyze_errors(preds: pd.DataFrame) -> List[str]:
    ...
    report.append(
        f"  Easiest to predict: {easiest} (MAPE={by_commodity.loc[easiest, 'MAPE']:.1f}%)"
    )
    ...
    normal = preds[
        (preds["rainfall_deviation"] >= p25) & (preds["rainfall_deviation"] <= p75)
    ]
    ...
    for commodity in preds["commodity"].unique():
        ...
        if len(resids) >= 5:
            ...
            report.append(
                f"  {commodity:10s}: mean_resid={resids.mean():+.2f}, t={t_stat:.2f}, p={p_val:.4f} → {biased}"
            )
...
def plot_error_analysis(preds: pd.DataFrame) -> None:
    ...
    ax = axes[0, 0]
    order = (
        preds.groupby("commodity")["pct_error"]
        .median()
        .sort_values(ascending=False)
        .index
    )
    sns.boxplot(
        data=preds, x="commodity", y="pct_error", order=order, ax=ax, palette="Set2"
    )
    ...
    for commodity in preds["commodity"].unique():
        ...
        ax.scatter(
            subset["actual"], subset["predicted"], s=20, alpha=0.6, label=commodity
        )
    lims = [
        preds[["actual", "predicted"]].min().min(),
        preds[["actual", "predicted"]].max().max(),
    ]
    ...
    ax = axes[1, 1]
    ax.scatter(
        preds["rainfall_deviation"], preds["abs_error"], s=15, alpha=0.5, c="steelblue"
    )
    ...
    if len(preds) > 10:
        z = np.polyfit(
            preds["rainfall_deviation"].dropna(), preds["abs_error"].dropna(), 1
        )
        p = np.poly1d(z)
        x_line = np.linspace(
            preds["rainfall_deviation"].min(), preds["rainfall_deviation"].max(), 100
        )
    ...
    time_error["date"] = pd.to_datetime(
        time_error["year"].astype(str) + "-" + time_error["month"].astype(str) + "-01"
    )
    ax.plot(
        time_error["date"],
        time_error["abs_error"],
        marker="o",
        markersize=3,
        linewidth=1.5,
        color="steelblue",
    ...
    plt.tight_layout()
    plt.savefig(
        VISUALIZATIONS / "figure_error_analysis.png", dpi=300, bbox_inches="tight"
    )
    plt.close()
...
def plot_residual_diagnostics(preds: pd.DataFrame) -> None:
    ...
    plt.tight_layout()
    plt.savefig(
        VISUALIZATIONS / "figure_residual_diagnostics.png", dpi=300, bbox_inches="tight"
    )
    plt.close()
