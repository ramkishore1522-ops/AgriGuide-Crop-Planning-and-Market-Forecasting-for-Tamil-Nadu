...
def load_and_prepare_data():
    ...
    if "actual" in tn_rainfall.columns:
        rainfall_monthly = (
            tn_rainfall.groupby(["year", "month"])
            .agg({"actual": "sum", "deviation": "mean"})
            .reset_index()
        )
        rainfall_monthly.columns = [
            "year",
            "month",
            "rainfall_mm",
            "rainfall_deviation",
...
def summarize_fold_metrics(fold_metrics):
    ...
    for model_name in sorted({row["Model"] for row in fold_metrics}):
        ...
        for metric in metrics:
            ...
            ci_half_width = (
                1.96 * std_value / np.sqrt(len(values)) if len(values) > 1 else 0.0
            )
...
def evaluate_baselines(df, features):
    ...
    model_specs = [
        ...
        (
            "Gradient Boosting",
            GradientBoostingRegressor(n_estimators=120, max_depth=5, random_state=42),
            False,
        ),
    ...
    for train_years, test_years in splits:
        ...
        for name, model, needs_scaling in model_specs:
            if name == "Hybrid Ensemble":
                ...
                y_pred = 0.5 * base_model.predict(
                    X_test_scaled if needs_scaling else X_test
                ) + 0.5 * ridge_model.predict(
                    X_test_scaled if needs_scaling else X_test
                )
            else:
...
def evaluate_optional_baselines(df, features):
    ...
    try:
        ...
        for commodity in sorted(df["commodity"].unique()):
            commodity_df = (
                df[df["commodity"] == commodity].sort_values(["year", "month"]).copy()
            )
            if len(commodity_df) < 24:
                continue
            commodity_df["date_index"] = pd.period_range(
                start="2015-01", periods=len(commodity_df), freq="M"
            )
            for train_years, test_years in get_time_series_splits(commodity_df):
                train_series = commodity_df[commodity_df["year"].isin(train_years)][
                    "price"
                ]
                test_series = commodity_df[commodity_df["year"].isin(test_years)][
                    "price"
                ]
                ...
                try:
                    ...
                    arima_metrics.append(
                        compute_regression_metrics(
                            test_series.values, np.asarray(forecast)
                        )
                    )
                except Exception:
                    continue
        if arima_metrics:
            results.append(
                summarize_baseline_results(arima_metrics, "ARIMA").iloc[0].to_dict()
            )
    ...
    try:
        ...
        for commodity in sorted(df["commodity"].unique()):
            ...
            if len(commodity_df) < 24:
                continue
            commodity_df["ds"] = pd.to_datetime(
                commodity_df["year"].astype(str)
                + "-"
                + commodity_df["month"].astype(str).str.zfill(2)
                + "-01"
            )
            ...
            try:
                ...
                prophet_metrics.append(
                    compute_regression_metrics(
                        commodity_df["y"].tail(3).values,
                        forecast["yhat"].tail(3).values,
                    )
                )
            except Exception:
                continue
        if prophet_metrics:
            results.append(
                summarize_baseline_results(prophet_metrics, "Prophet").iloc[0].to_dict()
            )
    except Exception as exc:
    ...
    try:
        ...
        for commodity in sorted(df["commodity"].unique()):
            ...
            X_train = (
                torch.tensor(np.array(X_train_seq, dtype=np.float32))
                .unsqueeze(-1)
                .to(device)
            )
            ...
            with torch.no_grad():
                for _ in range(len(test_scaled)):
                    seq = np.array(history[-lookback:], dtype=np.float32).reshape(
                        1, -1, 1
                    )
        ...
        if lstm_metrics:
            results.append(
                summarize_baseline_results(lstm_metrics, "LSTM").iloc[0].to_dict()
            )
    except Exception as exc:
    ...
    if results:
        ...
        print(
            "  No optional baselines were executed. Install the relevant packages to enable them."
        )
...
def run_ablation_studies(df):
    ...
    df_eval = (
        df.copy().sort_values(["commodity", "year", "month"]).reset_index(drop=True)
    )
    for lag in [1, 2, 3]:
    ...
    feature_groups = {
        ...
        "climate_plus_lag_plus_seasonality": [
            "year_trend",
            "month",
            "season",
            "is_monsoon",
    ...
    for name, features in feature_groups.items():
        ...
        row = {
            "Ablation": name,
            "Best_Model": (
                summary_df.sort_values("R2_mean", ascending=False).iloc[0]["Model"]
                if not summary_df.empty
                else "N/A"
            ),
        }
        row.update(
            summary_df.sort_values("R2_mean", ascending=False).iloc[0].to_dict()
            if not summary_df.empty
            else {}
        )
...
def train_models(df, features):
    ...
    for name, (model, use_scaled) in models.items():
        ...
        print(
            f"  {name:20s}: R2={metrics['R2']:.4f}, MAE=Rs.{metrics['MAE']:.2f}, RMSE={metrics['RMSE']:.2f}, MAPE={metrics['MAPE']:.2f}"
        )
...
def generate_report(
    results_df, importance, df, time_series_summary, ablation_df, optional_baselines
):
