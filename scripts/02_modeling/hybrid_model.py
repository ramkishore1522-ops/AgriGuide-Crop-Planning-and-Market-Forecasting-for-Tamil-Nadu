...
def get_arima_predictions(df: pd.DataFrame, train_mask, test_mask) -> np.ndarray:
    ...
    for commodity in df["commodity"].unique():
        comm_train = df[(df["commodity"] == commodity) & train_mask].sort_values(
            ["year", "month"]
        )
        comm_test = df[(df["commodity"] == commodity) & test_mask].sort_values(
            ["year", "month"]
        )
...
def expanding_window_evaluation(df: pd.DataFrame) -> Tuple[pd.DataFrame, Dict]:
        ...
        for model_name, config in models_config.items():
            if config["type"] == "naive":
                ...
                arima_train_preds = get_arima_predictions(
                    df,
                    df["year"].isin(
                        years[: max(2, fold_idx - 1)]
                    ),  # train ARIMA on earlier data
                    train_mask,
                )
    ...
    for name in models_config:
        if name != best_name and len(all_predictions[name]) == len(best_preds):
            try:
                abs_err_other = [
                    abs(a - p) for a, p in zip(all_actuals, all_predictions[name])
                ]
...
def visualize_results(results_df: pd.DataFrame, extras: Dict) -> None:
    ...
    plt.suptitle(
        "Model Performance Comparison (Expanding Window CV)",
        fontsize=14,
        fontweight="bold",
    )
    plt.tight_layout()
    plt.savefig(
        VISUALIZATIONS / "figure_hybrid_comparison.png", dpi=300, bbox_inches="tight"
    )
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(
        VISUALIZATIONS / "figure_hybrid_decomposition.png", dpi=300, bbox_inches="tight"
    )
    plt.close()
...
def generate_report(results_df: pd.DataFrame, extras: Dict) -> None:
    ...
    lines.append(
        "  Level 2: XGBoost meta-learner (combines ARIMA + climate + lag features)"
    )
