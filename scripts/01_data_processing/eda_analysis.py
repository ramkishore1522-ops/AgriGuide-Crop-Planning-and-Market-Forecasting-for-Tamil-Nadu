...
def load_data():
    ...
    data = {
        ...
        "rainfall": pd.read_csv(
            DATA_QUALITY / "rainfall_state_quality.csv", parse_dates=["date"]
        ),
        "groundwater": pd.read_csv(
            DATA_QUALITY / "groundwater_quality.csv", parse_dates=["date"]
        ),
        "vulnerability": pd.read_csv(DATA_QUALITY / "vulnerability_quality.csv"),
        "retail_prices": pd.read_csv(
            DATA_QUALITY / "retail_prices_quality.csv", parse_dates=["date"]
        ),
    }
...
def analyze_price_climate_correlation(data):
    ...
    rainfall_monthly = (
        rainfall.groupby(["state_name", "year_month"])
        .agg({"actual": "sum", "deviation": "mean"})
        .reset_index()
    )
    ...
    prices_monthly = (
        prices.groupby(["state_name", "commodity", "year_month"])
        .agg({"price": "mean"})
        .reset_index()
    )
    ...
    merged = pd.merge(
        prices_monthly, rainfall_monthly, on=["state_name", "year_month"], how="inner"
    )
    ...
    for commodity in key_commodities:
        ...
        if len(commodity_data) > 30:
            # Pearson correlation
            r_actual, p_actual = stats.pearsonr(
                commodity_data["actual"], commodity_data["price"]
            )
            r_dev, p_dev = stats.pearsonr(
                commodity_data["deviation"], commodity_data["price"]
            )

            # Spearman rank correlation
            rho_actual, p_rho_actual = stats.spearmanr(
                commodity_data["actual"], commodity_data["price"]
            )
            rho_dev, p_rho_dev = stats.spearmanr(
                commodity_data["deviation"], commodity_data["price"]
            )
    ...
    ax1.set_title(
        "Price Correlation with Rainfall Metrics", fontsize=12, fontweight="bold"
    )
    ...
    ax3.scatter(
        onion_data["deviation"], onion_data["price"], alpha=0.3, s=10, color="orange"
    )
    ...
    state_corr = (
        ...
        .apply(lambda x: x["price"].corr(x["deviation"]) if len(x) > 30 else np.nan)
        .dropna()
        .sort_values()
    )
    ...
    ax4.set_title(
        "States: Price Sensitivity to Rainfall Deviation",
        fontsize=12,
        fontweight="bold",
    )
    ...
    plt.tight_layout()
    plt.savefig(
        VISUALIZATIONS / "figure_3_price_climate_correlation.png",
        dpi=300,
        bbox_inches="tight",
    )
    plt.close()
...
def analyze_profitability(data):
    ...
    coc["profit_margin"] = (coc["net_profit"] / coc["revenue_per_hectare"] * 100).round(
        2
    )
    coc["cost_recovery"] = (coc["revenue_per_hectare"] / coc["cost_per_hectare"]).round(
        2
    )

    # Top profitable crops
    crop_profit = (
        coc.groupby("crop_name")
        .agg({"profit_margin": "mean", "net_profit": "mean", "cost_recovery": "mean"})
        .round(2)
    ...
    # State profitability
    state_profit = (
        coc.groupby("state_name")
        .agg(
            {"profit_margin": "mean", "net_profit": "mean", "cost_per_hectare": "mean"}
        )
        .round(2)
        .sort_values("profit_margin", ascending=False)
    )
    ...
    ax3 = axes[1, 0]
    crop_type_stats = (
        coc.groupby("crop_type")["cost_recovery"].mean().sort_values(ascending=False)
    )
    ...
    ax4.axvline(
        x=coc["profit_margin"].median(),
        color="green",
        linestyle="-",
        linewidth=2,
        label=f'Median: {coc["profit_margin"].median():.1f}%',
    )
    ...
    plt.tight_layout()
    plt.savefig(
        VISUALIZATIONS / "figure_7_profitability_analysis.png",
        dpi=300,
        bbox_inches="tight",
    )
    plt.close()
...
def analyze_vulnerability(data):
    ...
    rainfall_var = (
        rainfall.groupby("state_name")
        .agg({"actual": ["mean", "std"], "deviation": "mean"})
        .round(2)
    )
    rainfall_var.columns = ["avg_rainfall", "rainfall_std", "avg_deviation"]
    rainfall_var["rainfall_cv"] = (
        rainfall_var["rainfall_std"] / rainfall_var["avg_rainfall"] * 100
    ).round(2)
    ...
    composite = composite.merge(
        gw_latest.reset_index().rename(columns={"currentlevel": "groundwater_depth"}),
        on="state_name",
        how="left",
    )
    composite = composite.merge(
        income_stress.reset_index().rename(
            columns={"profit_margin_pct": "avg_profit_margin"}
        ),
        on="state_name",
        how="left",
    )
    ...
    composite["water_risk"] = normalize(
        composite["groundwater_depth"].fillna(composite["groundwater_depth"].median())
    )
    composite["rainfall_risk"] = normalize(composite["rainfall_cv"].fillna(0))
    composite["income_risk"] = normalize(
        -composite["avg_profit_margin"].fillna(0)
    ...
    colors = [
        "red" if x == "High Risk" else "orange" if x == "Medium Risk" else "green"
    ...
    ax1.set_title(
        "State Vulnerability Ranking (Top 20)", fontsize=12, fontweight="bold"
    )
    ax1.invert_yaxis()
    ...
    ax4 = axes[1, 1]
    ax4.scatter(
        composite["groundwater_depth"],
        composite["rainfall_cv"],
    ...
    for _, row in composite.head(5).iterrows():
        if pd.notna(row["groundwater_depth"]) and pd.notna(row["rainfall_cv"]):
            ax4.annotate(
                row["state_name"][:10],
                (row["groundwater_depth"], row["rainfall_cv"]),
                fontsize=8,
                alpha=0.8,
            )
    ...
    plt.tight_layout()
    plt.savefig(
        VISUALIZATIONS / "figure_8_vulnerability_assessment.png",
        dpi=300,
        bbox_inches="tight",
    )
    plt.close()
...
def generate_insights_report(price_climate, profitability, vulnerability):
    ...
    report.append(
        "- Vegetables (onion, tomato) are more sensitive to rainfall variability"
    )
    ...
    report.append("\nKey Findings:")
    report.append(
        f"- Average profit margin: {crop_profit['profit_margin'].mean():.1f}%"
    )
    report.append(
        f"- Crops with negative margins: {len(crop_profit[crop_profit['profit_margin'] < 0])}"
    )
    ...
    report.append(
        "2. WATER MANAGEMENT: Focus on groundwater recharge in depleting states"
    )
    report.append("3. PRICE SUPPORT: Strengthen MSP coverage for low-margin crops")
    report.append(
        "4. CLIMATE ADAPTATION: Early warning systems for price-sensitive commodities"
    )
