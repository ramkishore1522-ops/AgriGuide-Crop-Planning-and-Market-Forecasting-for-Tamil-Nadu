...
def check_stationarity(series: np.ndarray, name: str) -> Dict:
    ...
    if not HAS_STATSMODELS:
        return {'name': name, 'stationary': 'UNKNOWN', 'adf_stat': None, 'p_value': None}
...
def run_granger_all(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for commodity in KEY_COMMODITIES:
        ...
        if len(df_c) >= 20:
            ...
            rain_stat = check_stationarity(df_c['rainfall_mm'].values, f"{commodity}_rainfall")
            print(f"    Price stationary: {price_stat['stationary']} (p={price_stat['p_value']})")
            print(f"    Rainfall stationary: {rain_stat['stationary']} (p={rain_stat['p_value']})")
        ...
        if sig_results:
            ...
            print(f"    ✓ Rainfall → Price: SIGNIFICANT at lag {best['Lag (months)']} "
                  f"(F={best['F-statistic']:.2f}, p={best['p-value']:.4f})")
        else:
            print(f"    ✗ Rainfall → Price: NOT significant at any lag")

        sig_dev = [r for r in results_dev if r['Significant (p<0.05)'] == 'Yes']
        if sig_dev:
            best = min(sig_dev, key=lambda x: x['p-value'])
            print(f"    ✓ Deviation → Price: SIGNIFICANT at lag {best['Lag (months)']} "
                  f"(F={best['F-statistic']:.2f}, p={best['p-value']:.4f})")
        else:
            print(f"    ✗ Deviation → Price: NOT significant at any lag")
...
def visualize_granger(results_df: pd.DataFrame) -> None:
    ...
    for ax, climate_var, title in zip(
        ...
        pivot = subset.pivot(index='Commodity', columns='Lag (months)', values='p-value')
    ...
    plt.suptitle('Granger Causality Test Results (p-values)\n'
                 '*** p<0.01, ** p<0.05; green=significant',
                 fontsize=13, fontweight='bold')
    plt.tight_layout()
    plt.savefig(VISUALIZATIONS / 'figure_granger_causality.png', dpi=300, bbox_inches='tight')
    plt.close()
    ...
    if results_df.empty:
        ...
        if sig.empty:
            ...
            for _, row in sig.iterrows():
                lines.append(f"  {row['Climate Variable']:20s} → {row['Commodity']:10s} "
        ...
        lines.append(f"Significant (p<0.05): {sig_count} ({sig_count / total_tests * 100:.1f}%)")
        lines.append(f"Significant (p<0.01): {len(results_df[results_df['Significant (p<0.01)'] == 'Yes'])} tests")
