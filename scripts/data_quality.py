...
def validate_data(df, rules):
    ...
    for col, checks in rules.items():
        ...
        if 'min' in checks:
            ...
            if violations > 0:
                issues.append(f"'{col}': {violations:,} values below min ({checks['min']})")
        
        if 'max' in checks:
            violations = (df[col] > checks['max']).sum()
            if violations > 0:
                issues.append(f"'{col}': {violations:,} values above max ({checks['max']})")
...
def generate_quality_report(datasets_info):
    ...
    for name, info in datasets_info.items():
        ...
        report.append(f"  Missing values: {info['missing_before']:,} -> {info['missing_after']:,}")
...
# MAIN PROCESSING
# ============================================================
def process_dataset(filepath, name, missing_config, date_cols, numeric_cols, validation_rules, dedup_cols=None):
...
def main():
    ...
    df, info = process_dataset(
        ...
        date_cols=[],
        numeric_cols=['cul_cost_c2', 'main_product_value', 'derived_yield', 'profit_margin_pct'],
        validation_rules={
