...
def load_commodity_data() -> pd.DataFrame:
    ...
    try:
        ...
        tn_rain = rainfall[rainfall['state_name'].str.contains('Tamil', case=False, na=False)].copy()
...
def train_single_commodity(
    ...
    if not fold_df.empty:
        ...
        for model_name in models:
            if model_name != best_model_name and len(all_predictions[model_name]) == len(all_predictions[best_model_name]):
                try:
    ...
    for commodity in commodities_to_run:
        ...
        if not fold_df.empty:
            ...
            summary = fold_df.groupby('Model').agg(
                ...
                RMSE_mean=('RMSE', 'mean'), MAPE_mean=('MAPE', 'mean'),
...
def generate_shap_per_commodity(
    ...
    for idx, commodity in enumerate(commodities):
        ...
        try:
            ...
            shap.summary_plot(
                ...
                plot_size=None, max_display=len(FEATURES),
            )
            ...
            axes[idx].text(0.5, 0.5, f"{commodity}\nSHAP failed", ha='center', va='center')
    ...
    plt.suptitle('SHAP Feature Importance by Commodity', fontsize=14, fontweight='bold', y=1.02)
    plt.tight_layout()
    plt.savefig(VISUALIZATIONS / 'figure_per_commodity_shap.png', dpi=300, bbox_inches='tight')
    plt.close('all')
...
def generate_feature_importance_table(
    ...
    for commodity, (model, _scaler) in commodity_models.items():
        if hasattr(model, 'feature_importances_'):
            for feat, imp in zip(FEATURES, model.feature_importances_):
                rows.append({'Commodity': commodity, 'Feature': feat, 'Importance': round(imp, 4)})
    ...
    pivot = fi_df.pivot(index='Feature', columns='Commodity', values='Importance').fillna(0)
    ...
    ax.set_title('Feature Importance by Commodity (Without Commodity Feature)', fontweight='bold')
    ax.legend(title='Commodity', bbox_to_anchor=(1.05, 1), loc='upper left')
    plt.tight_layout()
    plt.savefig(VISUALIZATIONS / 'figure_feature_importance_per_commodity.png', dpi=300, bbox_inches='tight')
    plt.close()
...
def visualize_results(results_df: pd.DataFrame) -> None:
    ...
    summary = results_df.groupby(['Commodity', 'Model']).agg(
        R2_mean=('R2', 'mean'), R2_std=('R2', 'std'),
    ...
    plt.suptitle('Per-Commodity Model Performance (5-Fold TimeSeriesSplit)',
                 fontsize=14, fontweight='bold')
    plt.tight_layout()
    plt.savefig(VISUALIZATIONS / 'figure_per_commodity_performance.png', dpi=300, bbox_inches='tight')
    plt.close()
...
def generate_report(results_df: pd.DataFrame, fi_df: pd.DataFrame) -> None:
    ...
    best_per_commodity = (
        summary.loc[summary.groupby('Commodity')['R2_mean'].idxmax()]
    ...
    lines.append(f"NOTE: Commodity is NOT used as a feature.\n")
