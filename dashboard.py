...
@st.cache_data
def load_price_data():
    df = pd.read_csv(DATA_QUALITY / 'retail_prices_quality.csv')
    tn = df[df['state_name'] == 'Tamil Nadu'].copy()
...
# ─── Prediction Function ──────────────────────────────────────────────────────
def predict_price(model_data, commodity, month, year, rainfall_mm):
    ...
    if month in [6,7,8,9]:   season = 0
    elif month in [10,11]:    season = 1
    elif month in [12,1,2]:   season = 2
    else:                     season = 3
    ...
    features = [[commodity_encoded, year_trend, month, season, is_monsoon,
                 rainfall_mm, deviation, cat]]
    return model.predict(features)[0]

# ─── Sidebar ─────────────────────────────────────────────────────────────────
with st.sidebar:
    st.image("https://upload.wikimedia.org/wikipedia/commons/thumb/9/9c/Flag_of_Tamil_Nadu.svg/320px-Flag_of_Tamil_Nadu.svg.png", width=120)
    ...
    commodity = st.selectbox("🛒 Select Commodity",
        ['Rice','Tomato','Onion','Potato','Wheat','Milk','Sugar',
         'Gram Dal','Masoor Dal','Moong Dal','Urad Dal','Tur/Arhar Dal',
         'Groundnut Oil (Packed)','Mustard Oil (Packed)','Palm Oil (Packed)',
         'Soya Oil (Packed)','Sunflower Oil (Packed)','Vanaspati (Packed)',
         'Atta (Wheat)','Tea Loose','Salt Pack (Iodised)','Gur'], index=0)
...
st.caption(f"Powered by Gradient Boosting ML + Open-Meteo Live Weather | {district}, Tamil Nadu")
st.markdown("---")
...
with col2:
    st.metric("🌧️ Weekly Rainfall", f"{weather['weekly_rain']:.1f} mm" if weather else "N/A")
with col3:
...
with col_weather:
    ...
    if weather:
        ...
        fig_weather.add_trace(go.Bar(
            x=weather_df['Date'], y=weather_df['Rain (mm)'],
            name='Rain (mm)', marker_color='#44aacc', yaxis='y2'
        ))
        fig_weather.add_trace(go.Scatter(
            x=weather_df['Date'], y=weather_df['Max Temp (°C)'],
            name='Max Temp', line=dict(color='#ff6b6b', width=2)
        ))
        fig_weather.add_trace(go.Scatter(
            x=weather_df['Date'], y=weather_df['Min Temp (°C)'],
            name='Min Temp', line=dict(color='#74b9ff', width=2)
        ))
        fig_weather.update_layout(
            paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)',
            font=dict(color='white'), height=280, margin=dict(t=10,b=10),
...
if len(comm_data) > 0:
    ...
    monthly_avg['date_str'] = pd.to_datetime(monthly_avg[['year','month']].assign(day=1))
    monthly_avg = monthly_avg.sort_values('date_str')

    fig_trend = go.Figure()
    fig_trend.add_trace(go.Scatter(
        x=monthly_avg['date_str'], y=monthly_avg['price'],
    ...
    if commodity == 'Tomato':
        fig_trend.add_vrect(
            x0="2023-07-01", x1="2023-09-01",
            fillcolor="rgba(255,0,0,0.1)", line_width=0,
            annotation_text="2023 Crisis", annotation_position="top left"
        )
...
with col_all:
    ...
    fig_bar = px.bar(
        prices_df, x='Price (Rs/kg)', y='Commodity',
        orientation='h', color='Price (Rs/kg)',
...
fig_monthly.update_layout(
    ...
    xaxis_title='Month', yaxis_title='Price (Rs/kg)',
    margin=dict(t=10, b=10)
)
